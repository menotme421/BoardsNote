...
export const LinkCard = ({ node }: any) => {
  ...
  return (
    <div 
      className="w-[320px] bg-[var(--color-bg-primary)] border border-[var(--color-border)] rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer"
      onClick={() => window.open(node.content, '_blank')}
    >
      {loading ? (
        <div className="p-4 text-sm text-[var(--color-text-secondary)]">Loading preview...</div>
      ) : (
        <div className="p-4 flex flex-col gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[var(--color-text-secondary)] truncate">{metadata?.url}</span>
          </div>
